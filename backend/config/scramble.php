...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
            JSON REST API for storing images together with their raw EXIF/IPTC metadata.

            - No authentication.
            - All responses, errors included, are JSON (except the file download). Validation errors
              are Laravel's `422` structure: `{"message": "...", "errors": {"field": ["..."]}}`.
            - A request body over 10 MB is refused with a `413` before validation; smaller oversized
              uploads get a `422` naming the limit.
            - An unknown image ID, or any unknown route, is a `404` with `{"message": "Not found."}`.
            - The uploader's e-mail is stored, but never returned.
            - `temperature_c` (air temperature in Katowice at the upload hour) is filled in shortly
              after the upload by a background job; it stays `null` if the weather service fails.
            MD,
    ],
     ...
     */
    'dev_tools' => [
        // Tooling for developing Scramble itself; it would add a route of its own.
        'enabled' => false,
    ],
    ...
    'renderers' => [
        ...
        'elements' => [
            ...
            'hideSchemas' => false,
            'logo' => '',
            // No authentication, so "Try it" has no cookies to send.
    ...
    // Not "web": the API has no sessions, cookies or CSRF to set up for the docs page. Access
    // is granted by the viewApiDocs gate (AppServiceProvider).
    // The document is generated on each request unless `scramble:cache` has stored it (run it,
    // or `scramble:clear`, on deploy); the throttle bounds that cost on a public endpoint.
    'middleware' => [
